...
function clearInputErrorStyles() {
...
async function doRegister() {
  try {
    ...
    msgEl.style.color = '#0a7f6f';
    msgEl.textContent = '注册成功，正在返回登录页...';
    setTimeout(() => {
      window.location.href = '/login';
    }, 700);
  } catch (err) {
    msgEl.style.color = '#b13f00';
    msgEl.textContent = handleRegisterError(err);
  }
}
...
passwordConfirmEl.addEventListener('focus', () => {
  passwordConfirmEl.classList.remove('input-error');
});
...
if (togglePasswordConfirmBtn) {
  ...
  togglePasswordConfirmBtn.innerHTML = closedEyeSvg;

  togglePasswordConfirmBtn.addEventListener('click', () => {
    const isHidden = passwordConfirmEl.type === 'password';
    passwordConfirmEl.type = isHidden ? 'text' : 'password';
    togglePasswordConfirmBtn.innerHTML = isHidden ? openEyeSvg : closedEyeSvg;
  });
}
