...
class GPT2PerplexityCalculator:
    def __init__(self, config: AppConfig):
        ...
        try:
            ...
            raise RuntimeError("GPT-2 model initialization failed.")

        self.byte_encoder = bytes_to_unicode()
        self.byte_decoder = {v: k for k, v in self.byte_encoder.items()}

    @torch.inference_mode()  # 替代 no_grad，推理性能更好
    def calculate_perplexity(self, text: str) -> Tuple[float, int, List[float]]:
        ...
        try:
            ...
            logger.warning(f"Error calculating PPL: {e}")
            return 0.0, 0, []
...
class SingleSentencePredictor:
    def __init__(self, config: AppConfig):
        ...
        if not model_path.exists():
            raise FileNotFoundError(f"Head model not found at {model_path}")
    ...
    def predict_scores(self, sentences: List[str]) -> List[float]:
        ...
        final_scores = []
        for v in votes:
            if not v: final_scores.append(0.5); continue
            if len(v) <= 2: final_scores.append(sum(v) / len(v)); continue
...
def find_switch_index(results: List[dict], threshold: float) -> int:
    for idx, row in enumerate(results):
        if row["confidence"] >= threshold: return idx
    return len(results)


def main():
    parser = argparse.ArgumentParser(description="AI Generated Text Detector")
    ...
    try:
        ...
        for idx, (sent, score) in enumerate(zip(sents, scores)):
            ...
            rows.append({"index": idx, "text": sent, "label": label, "confidence": round(score, 4),
                         "ai_ratio": round(score, 4)})

        switch_idx = find_switch_index(rows, config.ai_threshold)
        print(json.dumps({"sentences": rows, "switch_sentence_index": switch_idx, "model_used": "v6-stable"},
                         ensure_ascii=False))
    except Exception as e:
